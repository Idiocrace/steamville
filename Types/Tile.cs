...
namespace TileGame.Types
{
    ...
    public class Tile
    {
        ...
        public string Serialize()
        {
            ...
            data["boundingBox"] = new Dictionary<string, float> // Changed int to float to match Vector2
            {
                { "width", BoundingBox.X },
                            ...
                            foreach (var contProp in caseObj.EnumerateObject())
                            {
                                ...
                                if (contProp.Value.ValueKind == System.Text.Json.JsonValueKind.Object)
                                {
                                    foreach (var resProp in contProp.Value.EnumerateObject())
                                    {
                                        ...
                                        // FIX IS HERE: We explicitly cast 'r' to 'Resource'
                                        var resource = TileGame.Resources.FirstOrDefault(r => ((Resource)r).ID == resId);
                                        ...
                                        resDict[(Resource)resource] = amount;
                                    }
                                }
